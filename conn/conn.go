...
func Listen(addr, typ string, tlsCfg *tls.Config) (l *Listener, err error) {
	...
	go func() {
		for {
			...
			newConn, ok := rawConn.(*net.TCPConn)
			if ok {
				newErr := newConn.SetKeepAlive(true)
				if newErr != nil {
					log.Error("Failed to set keep a live %s", newErr.Error())
				}
				newErr = newConn.SetKeepAlivePeriod(time.Minute)
				if newErr != nil {
					log.Error("Failed to set keep a live %s", newErr.Error())
				}
			}
...
func Join(c Conn, c2 Conn) (int64, int64) {
	...
	pipe := func(to Conn, from Conn, bytesCopied *int64) {
		...
		buf := util.GetBuf(16 * 1024)
		defer util.PutBuf(buf)
		*bytesCopied, _ = io.CopyBuffer(to, from, buf)
