...
func init() {
	...
	if keenApiKey != "" {
		metrics = NewKeenIoMetrics(60 * time.Minute)
	} else {
		metrics = NewLocalMetrics(30 * time.Minute)
	}
}
...
func (m *LocalMetrics) CloseTunnel(t *Tunnel) {
	...
	switch t.ctl.auth.OS {
	case "windows":
		m.windowsCounter.Dec(1)
	case "linux":
		m.linuxCounter.Dec(1)
	case "darwin":
		m.osxCounter.Dec(1)
	default:
		m.otherCounter.Dec(1)
	}
...
func (m *LocalMetrics) Msg() ([]byte, error) {
...
func (k *KeenIoMetrics) Msg() (bf []byte, err error) {
	return
}
