...
func (t *Tunnel) listenTcp(listener *net.TCPListener) {
	for {
		...
		newErr := tcpConn.SetKeepAlive(true)
		if newErr != nil {
			log.Error("Failed to set keep a live %s", newErr.Error())
		}
		newErr = tcpConn.SetKeepAlivePeriod(time.Minute)
		if newErr != nil {
			log.Error("Failed to set keep a live %s", newErr.Error())
		}
		if err != nil {
...
func (t *Tunnel) HandlePublicConnection(publicConn conn.Conn) {
	...
	proxyConn, err := t.GetWorkConnFromPool(publicConn.RemoteAddr().String())
...
func (t *Tunnel) GetWorkConnFromPool(dstAddr string) (proxyConn conn.Conn, err error) {
	...
	for i := 0; i < (1 + proxyMaxPoolSize); i++ {
		...
		if err = msg.WriteMsg(proxyConn, startPxyMsg); err != nil {
			_ = proxyConn.Warn("Failed to write StartProxyMessage: %v, attempt %d, message %s", err, i, startPxyMsg)
			_ = proxyConn.Close()
		} else {
			break
		}
	...
	if err != nil {
		_ = proxyConn.Error("try to get work connection failed in the end")
		return
	}
