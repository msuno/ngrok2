...
func NewControl(ctlConn conn.Conn, authMsg *msg.Auth) {
	...
	subdomain, err := CheckAuth(authMsg.User)
	if err != nil {
		failAuth(err)
		return
	}
	...
	if authMsg.Version != version.Proto {
		err := fmt.Errorf("incompatible versions. Server %s, client %s. Download a new version at http://ngrok.com", version.MajorMinor(), authMsg.Version)
		failAuth(err)
		return
	}
...
func (c *Control) manager() {
	...
	for {
		select {
		case <-reap.C:
			if time.Since(c.lastPing) > (pingTimeoutInterval + time.Second) {
...
func (c *Control) GetProxy() (proxyConn conn.Conn, err error) {
	...
	default:
		...
		case <-time.After(pingTimeoutInterval + time.Second):
